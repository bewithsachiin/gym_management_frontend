...
exports.getAllBookings = async (req, res, next) => {
  ...
  try {
    ...
    console.log(`✅ PT:getAllBookings → ${sessions.length} sessions found`);
...
exports.getBookingById = async (req, res, next) => {
  ...
  try {
    ...
    console.log("📌 PT:getBookingById → Session:", session ? {
      id: session.id,
    ...
    if (userRole === "member" && session.memberId !== req.user.id) {
      console.warn("⛔ PT:getBookingById → Member tried to access someone else's booking");
...
exports.createBooking = async (req, res, next) => {
  ...
  try {
    ...
    if (!finalBranchId) {
      console.warn("⚠️ PT:createBooking → Branch ID missing");
      return next(new Error("Branch ID missing"));
    }

    // 🟦 Validate Trainer
    const trainer = await prisma.user.findUnique({ where: { id: parseInt(trainerId) } });
    console.log("👨‍🏫 Trainer Check:", trainer ? { id: trainer.id, branchId: trainer.branchId } : null);
    ...
    console.log("🧍 Member Check:", member ? { id: member.id, branchId: member.branchId } : null);
...
exports.updateBooking = async (req, res, next) => {
  ...
  try {
    ...
    if (userRole === "personaltrainer" && session.trainerId !== req.user.id) {
      console.warn("⛔ PT:updateBooking → Trainer tried to edit other's session");
      return next(new Error("Access denied"));
    }
...
exports.deleteBooking = async (req, res, next) => {
  ...
  try {
    ...
    if (userRole === "personaltrainer" && session.trainerId !== req.user.id) {
      console.warn("⛔ PT:deleteBooking → Trainer tried to delete other's session");
      return next(new Error("Access denied"));
    }
    ...
    console.log("✅ PT:deleteBooking → Deleted sessionId:", id);
